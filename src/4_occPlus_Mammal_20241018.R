...
#############################################################################
# for obtaining Conditional posterior probabilities (COP)
...
n <- length(unique(data$info$Site))
S <- ncol(data$OTU)

logpost_samples <- matrix_of_draws[,grepl("log_post_pocc\\[", colnames(matrix_of_draws))]
logitpostp1m_samples <- matrix_of_draws[,grepl("log_post_p1mocc\\[", colnames(matrix_of_draws))]

niter <- nrow(logpost_samples)

logpost_samples_array <- array(logpost_samples, dim = c(niter, n, S))
logpost1m_samples_array <- array(logitpostp1m_samples, dim = c(niter, n, S))

postoccprobs <- exp(logpost_samples_array) / (exp(logpost_samples_array) + exp(logpost1m_samples_array))
...
write.csv(occ, here("outputs", "occPlus_outputs_Viorel covs", "Mammal_outputs_20241018_4factors", "COPoccupancybysite_occPlus_Mammal_20241018_4factors.csv"))
