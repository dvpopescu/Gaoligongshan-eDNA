...
runOccPlus <- function(data,
  ...
  # model <- stan_model(file = "C:/Users/viore/Dropbox/Columbia/KIZ/Gaoligongshan/glgs_20250702J/code_pcr_ccord2.stan", verbose = T)
  ...
  vb_fit <- vb(model, data = occ_dat,
               algorithm = "meanfield",
               pars = c("beta_psi","beta0_psi",
                        "beta_ord","beta_theta",
                        "p","q","theta0","logit_psi",
                        "U","L","E","log_lik",
                        # "log_theta",
                        "log_post_pocc", "log_post_p1mocc"#,
                        # "log_psi", "log1m_psi"
                        ),
               elbo_samples = 500,
               init = init_fun,
